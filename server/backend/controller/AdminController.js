...
AdminController.prototype.actionImport = function (model) {
    var attributes = this.req.query.data || this.req.body.data;
    var jobj = JSON.parse(attributes);
    var _pid = this.req.query.pid || this.req.body.pid;
    return this.model.insertMany(jobj).then(r => {
        return this.model.findAll({ pid: _pid }).then(rs => {
            return {
                code: 200,
                attributes: rs,
            }
...
AdminController.prototype.put_actionReadexcel = function (model) {
...
AdminController.prototype.actionDelete = async function (model) {
    ...
    if (this.req.role[table_name + '_delete']) {
        if (typeof (id) == 'object') {
            ...
            var rs = await this.model.findOne(id, true);
