...
db.system.js.save({
    _id: 'out_facebook_group_school_1',
    value: function (group_ids = false) {
        var search = group_ids && group_ids.length ? { _id: { $in: group_ids } } : {};
        search['$and'] = [
            { 'list_keyword': { $in: db.getCollection('ref_common').find({ type: "school_1" })[0].keyword } },
        ];
        var list = [];
        db.getCollection('facebook_group').find(search).forEach(function (item) {
            ...
            if (list.length == 10000) {
                insert_data_ignore(list, 'facebook_group_school_1');
                list = [];
            }
        })
        if (list.length) {
...
db.system.js.save({
    _id: 'build_list_keyword_tu_dien_facebook_group_school_1',
    value: function (group_ids = false) {
        build_facebook_education_list_keyword_tu_dien();
        var search = group_ids && group_ids.length ? { _id: { $in: group_ids } } : {};
        search['list_keyword_tu_dien'] = {$exists: false};
        var list = [];
        db.getCollection('facebook_group_school_1').find(search).forEach(function (item) {
            list.push({
                updateOne: {
                    filter  : {_id: item._id},
            ...
            if (list.length == 10000) {
                db.getCollection('facebook_group_school_1').bulkWrite(list);
                list = [];
            }
        })
        if (list.length) {
...
// function update_city_id_facebook_group_school_1_by_hometown
db.system.js.save({
    _id: "update_city_id_facebook_group_school_1_by_hometown",
    value: function (group_ids = false) {
        var search = group_ids && group_ids.length ? {_id: {$in: group_ids}} : {};
...
// function update_city_id_facebook_group_school_1_by_hometown_by_status
db.system.js.save({
    _id: "update_city_id_facebook_group_school_1_by_hometown_by_status",
...
// function insight_facebook_group_school_1
db.system.js.save({
    _id: "insight_facebook_group_school_1",
    value: function (school_1_ids = false, group_ids = false) {
        ...
        db.getCollection('facebook_group_school_1').aggregate([
            ...
            {$project:{_id: 1,name:1,alias: 1, sum:1, list_keyword_tu_dien:1, hometown_city_id: 1,hometown_city_name: 1, ref_school_1:{
                $map: { 
                    "input": "$ref_school_1", 
                    "as": "m", 
                    "in": { 
            ...
            if(item.hometown_city_id) {
                ...
                for(var it_school_1 of item.ref_school_1) {
                    if(item.hometown_city_id ==it_school_1.city_id) {
                        ref_school_1.push(it_school_1);
                    }
                }
                if(ref_school_1.length > 1) {
                    ref_school_1 = trim_school_name(ref_school_1,'list_keyword_tu_dien', item);
                }
                if(ref_school_1.length == 1) {
            ...
            if(i % 10000 == 0) {
                if(list.length) {
                    db.getCollection('facebook_insight_group_school_1').bulkWrite(list);
                }
...
// function out_user_insight_group_school_1
db.system.js.save({
    _id: "out_user_insight_group_school_1",
    value: function (data_ids = false, facebook_ids = false) {
        ...
        if(facebook_ids && facebook_ids.length) {
            ...
            var list_ids = [];
            db.getCollection('facebook_insight_group_school_1').find(search_ref).forEach(function(item){
        ...
        db.getCollection('facebook_user_group').aggregate([
            {$match: search},
            {$lookup:{
                    from                : 'facebook_insight_group_school_1',
                    localField          : 'data',
                    foreignField        : '_id',
                    as                  : 'facebook_insight_group_school_1',
            }},
            {$project:{_id: 1, facebook_insight_group_school_1:1 }},
        ]).forEach(function(item){
            i++;
            if(item.facebook_insight_group_school_1.length)  {
                for(var it of item.facebook_insight_group_school_1) {
            ...
            if(i % 10000 == 0) {
                db.getCollection('user_insight_group_school_1').bulkWrite(list);
