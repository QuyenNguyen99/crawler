...
db_58.getCollection('user_infer').find({hometown_infer:{$ne:""}}).noCursorTimeout().forEach(function(item){
    list.push({
            _id : "" + item.face_id,
            city_id : ref_city[item.hometown_infer]._id,
            city_name: ref_city[item.hometown_infer].name,
    });
    if(list.length == 10000) {
        db_57.getCollection('user_hometown_infer').insertMany(list);
        list = [];
    }
})
