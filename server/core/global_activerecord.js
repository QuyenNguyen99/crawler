...
var db = new DB.Adapter(CONFIG.MYSQL[CONFIG.SERVER['crawlersystem']]);
...
GlobalActiveRecord.prototype.db_key = CONFIG.SERVER['crawlersystem'];
GlobalActiveRecord.prototype.project_key = 'crawlersystem';
...
GlobalActiveRecord.prototype.findAll = async function (condition, params) {
    ...
    if ((this.hasAttribute('is_delete') || this.hasAttribute('IS_DELETE')) && !CONFIG.MONGO[this.db_key]) { this.db.where(this.tableName() + '.is_delete', '0'); }
    var rs = await this.db.get(this.tableName());
    if (this.tableName() == "job_type") {
        //console.log(this.db);
        //console.log(rs);
    }
...
GlobalActiveRecord.prototype.joinTableByAttribute = function (attr, type = 'INNER', ON = false) {
    if (ON) {
        this.db.join(attr, ON, type);
    } else if (attr.match(/_(mul|MUL)$/gi)) {
        return this.joinTableMulByAttribute(attr, type, ON);
    } else if (attr.match(/^list_/gi) && !(this.rule[attr] && this.rule[attr]['not_list']) && CONFIG.MYSQL[this.db_key]) {
        return this.joinTableListByAttribute(attr, type, ON);
    } else if (this.rule[attr] && this.rule[attr]['fk']) {
        var fk = this.rule[attr]['fk'];
        var table_fk = fk['table'];
        var ref_id = fk['ref_id'];
...
GlobalActiveRecord.prototype.joinTableMulByAttribute = function (attr, type = 'INNER', ON = false) {
    if (ON) {
        this.db.join(attr, ON, type);
        return this.buid_join(type, attr, ON);
    } else if (this.rule[attr]) {
...
GlobalActiveRecord.prototype.joinTableListByAttribute = function (attr, type = 'INNER', ON = false) {
    if (ON) {
        this.db.join(attr, ON, type);
        return this.buid_join(type, attr, ON);
    } else if (this.rule[attr]) {
            ...
            if (that.RULE[v]['fk']['join']) {
                ...
                if (that.RULE[v]['fk']['join'][0] && typeof (that.RULE[v]['fk']['join'][0]) == 'object') {
                    ...
                    for (var i in that.RULE[v]['fk']['join']) {
                        ...
                        this.joinTableByAttribute(item[0], item[1], item[2]);
                    ...
                    this.joinTableByAttribute(that.RULE[v]['fk']['join'][0], that.RULE[v]['fk']['join'][1]);
