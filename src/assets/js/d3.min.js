    ...
    d3.netJsonGraph = function(url, opts) {
        ...
        opts = d3._extend({
            ...
            onClickNode: function(n) {
                var link_web = location.hostname.indexOf('localhost') >= 0 ? 'http://localhost:6800' : (location.protocol + '//' + location.hostname + '/apicyberinsights');
                console.log(url);
                link_web = link_web.replace(/\/$/gi, '');
                if(!url.includes('http'))
                {
                    url = link_web + url;
                }
                var url_ = new URL(url);
                var tok = url_.searchParams.get("token");
                $.get(link_web+"/cyberinsights/facebook/getfacebookprofile?id="+n.id + "&token=" + tok, function(data, status){
                    var html = 
                    "<div class=\"msb_result\">"+
                        "<div class=\"msb_result_avatar\" style=\"text-align: center;\">"+
                            "<h3>Node</h3>"+
                            "<a href=\"/profilefacebook/"+n.id+"\" target=\"_blank\" tooltip=\"Địa chỉ facebook\">"+
                            "<img style=\"border-radius: 100%;\" src=\"http://graph.facebook.com/"+n.id+"/picture?type=normal\" class=\"Avatar\"></a>"+
                    ...
                    var overlay = d3.select(".njg-overlay"),
                        ...
                        if(n.properties) {
                            for(var key in n.properties) {
                                ...
                                html +="<a target=\"_blank\" href='https://www.facebook.com/"+n.properties[key]+"' tooltip=\""+key.replace(/_/g, " ")+"\">"+
                                "<img style=\"border-radius: 100%;\" src=\"http://graph.facebook.com/"+n.properties[key]+"/picture?type=normal\" class=\"Avatar\"></a>";
                        }
                    }
                    if(n.linkCount) { html += "<p style='text-align: center;padding: 5px;'><b>Số lượng kết nối (sample)</b>: " + n.linkCount/2 + "</p>"; }
            ...
            onClickLink: function(l) {
                var overlay = d3.select(".njg-overlay"),
                    overlayInner = d3.select(".njg-overlay > .njg-inner"),
                html = "<h3>Kết nối</h3>";
                html +="<p><a target=\"_blank\" href='/profilefacebook/"+(l.source.label || l.source.id)+"' tooltip=\"Nguồn\">"+
                            "<img style=\"border-radius: 100%;\" src=\"http://graph.facebook.com/"+(l.source.label || l.source.id)+"/picture?type=small\" class=\"Avatar\"></a>";

                html +="<a target=\"_blank\" href='/profilefacebook/"+(l.target.label || l.target.id)+"' tooltip=\"Đích\">"+
                        "<img style=\"border-radius: 100%;\" src=\"http://graph.facebook.com/"+(l.target.label || l.target.id)+"/picture?type=small\" class=\"Avatar\"></a></p>";
                if(l.properties) {
        ...
        var el = d3.select(opts.el).style("position", "relative"),
            ...
            processJson = function(graph) {
                ...
                if(opts.defaultStyle) {
                    ...
                    node.style({
                        "fill": function(d){ 
                            if(d.linkCount/2/graph["nodesample"]>0.8)
                            {
                                return "#E6550D";
                            }
                            else if(d.linkCount/2/graph["nodesample"]>0.5){
                                return "#31A354";
                            }
                            return "#3182BD"; },
                        "cursor": "pointer"
                    ...
                    //nodes.length
                    node.attr("r", function(d){ return (d.linkCount/2/graph["nodesample"]) * 16 });
                }
                // Metadata style
                if(opts.metadata) {
                ...
                var attrs_ = [
                            ...
                            ["node","Node"],
                            ["linkmax","Kết nối"],
                            ["nodemax","Kết nối lớn nhất"],
                            ["nodesample","Kết nối lớn nhất (tập mẫu)"]]
                    html = "";
                if(graph.label) {
                    html += "<h3 style='padding-bottom: 20px;color: #000;'>" + graph.label + "</h3>";
                }
                ...
                for(var i in attrs_) {
                    ...
                    if(graph[attr[0]]) {
                        if(attr[0]=="dense")
                        {
                            html += "<h1><b>" + attr[1] + "</b>: <span>" + graph[attr[0]] + "</span></h1><br/>";
                        }
                        else if(attr[0]=="nodesample")
                        {
                            //html += "<p style='margin: 0px;'><small><b>" + attr[1] + "</b>: <span>" + graph[attr[0]] + "</span></small></p>";
                        }
                        else
                        {
                            html += "<p><b>" + attr[1] + "</b>: <span>" + graph[attr[0]] + "</span></p>";
                        }
                    }
                }
                //html += "<p style='margin: 0px;'><small><b>Node (tập mẫu)</b>: <span>" + graph.nodes.length + "</span></small></p>";
                //html += "<p style='margin: 0px;'><small><b>Kết nối (tập mẫu)</b>: <span>" + graph.links.length + "</span></small></p>";
                html += "<p style='margin: 0px;'><small><i>Node lượng kết nối / số kết nối nhiều nhất(tập mẫu):</i></small></p>";
                html += "<p><span style='border-radius:50%;background:#E6550D;width: 20px;height: 20px;display: inline-block;'></span> > 0.7</p>";
                html += "<p><span style='border-radius:50%;background:#31A354;width: 20px;height: 20px;display: inline-block;'></span> > 0.5</p>";
                html += "<p><span style='border-radius:50%;background:#3182BD;width: 20px;height: 20px;display: inline-block;'></span> < 0.5</p>";
